...
function preset(name, amp, pd, slot = null, cardDirty = false, pitch = null, pitchSource = null, pitch2 = null, pd2 = null) {
  ...
  try {
    ...
    if (Array.isArray(saved)) {
      const custom = saved
        ...
        .map((item) => constrainPitchToEnvelope(preset(item.name || "Custom preset", item.amp, item.pd, item.slot, item.cardDirty, item.pitch, item.pitchSource, item.pitch2, item.pd2)))
...
function duplicateFactoryPreset() {
  ...
  presets.push(preset(`${source.name} copy`, source.amp, source.pd, null, false, source.pitch, source.pitchSource, source.pitch2, source.pd2));
  selected = presets.length - 1;
  savePresets();
  render();
  return true;
...
function drawCurves() {
  ...
  ctx.fillText(`PD1`, 62, 22);
  ctx.fillStyle = theme.pd2;
  ctx.fillText(`PD2`, 106, 22);
  ctx.fillStyle = theme.muted;
...
function reconcileCardEnvelopes(mask, cardEnvelopes) {
  ...
  cardEnvelopes.forEach((cardEnvelope, slot) => {
    ...
    const cardPreset = constrainPitchToEnvelope(preset(`Card Envelope ${slot + 1}`, cardEnvelope.amp, cardEnvelope.pd, slot, false, cardEnvelope.pitch, null, cardEnvelope.pitch2, cardEnvelope.pd2 || cardEnvelope.pd));
    if (customPresetCount() >= MAX_BROWSER_CUSTOM_PRESETS) {
...
function finishEnvelopeRead() {
  ...
  if (session.reason === "delete") {
    ...
    setStatus(deleted
      ? `Verified card slot ${session.slot + 1} is empty. Any matching dual-oscillator card-slot draft was removed.`
  ...
  const emptyNote = count === 0
    ? " No saved card envelopes were reported; any remaining presets are dual-oscillator browser drafts."
    : "";
  if (count === 0 && session.reason === "manual") {
    selected = Math.min(3, presets.length - 1);
    render();
...
function downloadJson() {
  ...
  link.href = url;
  link.download = "c1zzl3-dual-oscillator-lanes-presets.json";
  link.click();
