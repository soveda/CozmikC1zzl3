...
const ENVELOPE_LAB_WINDOW_NAME = "c1zzl3-pitch-envelope-lab";
const WAVE_FAMILIES = [
...
function renderDraft(draft) {
  ...
  el.mappedDraftBox.textContent = `8-wave family ${draft.wave.label}, CZ DCA -> C1ZZL3 amplitude, CZ DCW -> C1ZZL3 phase distortion, and CZ DCO pitch envelopes are decoded for experimental review.`;
...
el.openDraft.addEventListener("click", () => {
  if (handoffDraft(currentDraft)) {
    setStatus("Draft handed off to Pitch Envelope Lab without adding it to the URL.", "ok");
    openEditorTab();
  } else {
