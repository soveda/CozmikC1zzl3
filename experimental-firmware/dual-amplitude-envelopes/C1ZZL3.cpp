...
static constexpr uint8_t WebMidiEnvelopeProtocolVersion = 5u;
...
static constexpr uint32_t WebMidiMaxSysexLength = 276u;
...
private:
    ...
    static constexpr uint16_t CustomEnvelopeSaveVersion = 5;
        ...
        int32_t noiseAmt)
    {
        ...
        int32_t pd1 = applyEnvelopeToPd(pd, pdEnvelopeLevel);
        int32_t pd2 = applyEnvelopeToPd(pd, pd2EnvelopeLevel);

        int32_t osc1 =
            oscCZ(phase1, freq1, pd1, wave, noiseAmt);

        int32_t freq2 =
            applyPitchEnvelopeToFrequency(applyDetune(freq, osc2Detune), pitch2EnvelopeLevel);

        int32_t osc2 =
            oscCZ(phase2, freq2, pd2, wave, noiseAmt);
    ...
    void handleWebMidiEnvelope(bool persist)
    {
        ...
        if (!legacyPayload && !singlePitchPayload && !dualPitchPayload && !dualOscPayload && !dualAmpPayload)
            return;
        ...
        else
        {
            ...
            if (dualPitchPayload || dualOscPayload || dualAmpPayload)
            {
                for (uint32_t i = 0; i < 8u; ++i)
                {
                    uint16_t level = decodeWebMidiUint14(offset);
        ...
        if (dualOscPayload || dualAmpPayload)
        {
            for (uint32_t i = 0; i < 8u; ++i)
            {
                uint16_t level = decodeWebMidiUint14(offset);
