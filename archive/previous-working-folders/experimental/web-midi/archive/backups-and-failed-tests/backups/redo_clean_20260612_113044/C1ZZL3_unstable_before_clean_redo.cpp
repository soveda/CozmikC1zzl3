...
public:
    ...
    void ProcessSample() override
    {
        ...
        if (mode != Switch::Up)
        {
            ...
            int32_t pd = clampControlWithCv(pdControl, cv1);
            int32_t wave = clampControlWithCv(waveControl, cv2);
...
private:
    ...
    static constexpr uint32_t WaveTableCount = 8;
    static constexpr uint32_t WaveTableSize = 256;
    static constexpr uint32_t WaveTableMask = WaveTableSize - 1;
        ...
        int32_t noiseAmt)
    {
        ...
        int32_t osc2 = 0;
        bool needOsc2 = osc2Level > 0 || ring > 0;
        if (needOsc2)
        {
            int32_t freq2 =
...
private:
    ...
    inline int32_t morphWave(uint32_t phase, int32_t wave)
    {
        ...
        int32_t a = waveTableSample(phase, index);
        int32_t b = waveTableSample(phase, index < 7 ? index + 1 : 7);

        return a + (((b - a) * (int32_t)frac) >> 12);
    }

    inline int32_t waveTableSample(uint32_t phase, uint32_t wave)
    {
        wave &= 7u;
        uint32_t index = (phase >> 24) & WaveTableMask;
        uint32_t frac = (phase >> 12) & 4095u;

        int32_t a = waveTable[wave][index];
        int32_t b = waveTable[wave][(index + 1u) & WaveTableMask];
    ...
    {
        for (uint32_t wave = 0; wave < WaveTableCount; ++wave)
        {
            for (uint32_t i = 0; i < WaveTableSize; ++i)
            {
                uint32_t phase = i << 24;
                waveTable[wave][i] = (int16_t)czWave(phase, wave);
            }
        }
    }
    ...
    int32_t clampControlWithCv(int32_t control, int32_t cv)
    {
        int32_t modulation = cv << 1;
        int32_t value = control + modulation;

        if ((modulation > 0 && value < control) ||
            (modulation < 0 && value > control))
            return modulation > 0 ? 4095 : 0;
...
private:
    ...
    int16_t waveTable[WaveTableCount][WaveTableSize] = {};
