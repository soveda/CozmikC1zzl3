...
private:
        ...
        int32_t noiseAmt)
    {
        ...
        noiseAmt = safeNoiseControl(noiseAmt, wave);
    ...
    int32_t safeNoiseControl(int32_t noiseAmt, int32_t wave)
    {
        noiseAmt = clamp12(noiseAmt);

        if (!midiOrEnvelopeActive())
            return noiseAmt;

        int32_t cap = wave > 3000 ? 512 : 1024;
        if (noiseAmt > cap)
            return cap;
    ...
    bool midiOrEnvelopeActive()
    {
        return midiNoteActive ||
            (envelopePreset != (uint8_t)EnvelopePreset::Off && envelopeActive);
    }
...
private:
    ...
    void applyPendingWebEnvelope()
    {
        ...
        if (shouldAutoAuditionWebEnvelope())
            triggerEnvelope();
    ...
    bool shouldAutoAuditionWebEnvelope()
    {
        return pdControl < 3400 || waveControl < 3600;
    }
    ...
    inline int32_t morphWave(uint32_t phase, int32_t wave)
    {
        ...
        int32_t a = activeCzWave(phase, index);
        int32_t b = activeCzWave(phase, index < 7 ? index + 1 : 7);

        return a + (((b - a) * (int32_t)frac) >> 12);
    }

    inline int32_t activeCzWave(uint32_t phase, uint32_t wave)
    {
        if (midiOrEnvelopeActive() && wave >= 5)
            return lightResonantWave(phase, wave);

        return czWave(phase, wave);
    }
    ...
    inline int32_t lightResonantWave(uint32_t phase, uint32_t wave)
    {
        uint32_t p = (phase >> 20) & 4095;
        int32_t base = ((int32_t)p) - 2048;
        int32_t folded = p < 2048 ?
            ((int32_t)p << 1) - 2048 :
            6143 - ((int32_t)p << 1);

        switch (wave)
        {
            case 5: return (base + (folded >> 1)) >> 1;
            case 6: return folded;
            default: return (base >> 1) + (folded >> 1);
        }
    }
    ...
    void updateTuringMachine(
        int32_t mutationControl,
        int32_t lengthControl,
        int32_t clockSpeedControl,
        bool triggerEnvelopeOnStep)
    {
        uint32_t previousTuringLength = turingLength;
        turingLength = 2 + ((lengthControl * 14) >> 12);
        ...
        else
        {
            if (externalClockAge < 96000u)
                ...
                clocked = internalTuringClock(clockSpeedControl);
        }

        if (clocked)
        {
            stepTuring(mutationControl);
            if (triggerEnvelopeOnStep)
    ...
    bool internalTuringClock(int32_t speed)
    {
        ...
        if (moved > 64)
        {
            ...
            turingClockPeriodReady = false;
        }

        if (!useTappedClock && (!turingClockPeriodReady || speed != lastClockSpeed))
        {
    ...
    int32_t applyDetune(int32_t freq, int32_t detune)
    {
        ...
        int64_t fine = (bend * bend) >> 11;
        int64_t offset = ((int64_t)freq * fine) >> 12;
        int64_t detuned = (int64_t)freq + (sign * offset);

        if (detuned < 0)
            return 0;

        if (detuned > 0x7FFFFFFF)
            return 0x7FFFFFFF;
...
private:
    ...
    bool turingClockPeriodReady = false;
